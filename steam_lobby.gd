...
const lobby_cache_path: String = "user://lobby.txt"
...
var lobby_id: int = 0:
	set(new_lobby_id):
		...
		if lobby_id == 0:
			...
			print("Removed lobby_cache")
		else:
			var file: FileAccess = FileAccess.open(lobby_cache_path, FileAccess.WRITE)
			if not file: push_error("Something went wrong opening lobby cache."); return
			file.store_64(lobby_id)
			print("Stored %d" % lobby_id)
			file.close()
...
var lobby_data: LobbyData:
	set(new_lobby_data):
		if lobby_data: lobby_data.game_started.disconnect(_connect_to_game)
		if new_lobby_data: new_lobby_data.game_started.connect(_connect_to_game)
...
func _load_lobby_id_from_cache() -> void:
	...
	if not file: push_error("Something went wrong while reading the lobby cache.")
...
## Will update the Steam Lobby data so the other members receive it.
...
## React to the lobby data being updated by filling the lobby_data.
func _lobby_data_updated(_success: int, _lobby_id: int, _member_id: int) -> void:
	_fill_lobby_data()

## Fill the LobbyData object with well lobby data.
func _fill_lobby_data() -> void:
	if not lobby_data: push_error("No lobby_data was set.")

	var data: Dictionary = Steam.getAllLobbyData(lobby_id)
	lobby_data.update_data(data)
	lobby_data_changed.emit(lobby_data)

## Changes the 3D Scene to the Game Scene.
func _connect_to_game() -> void:
	# If you are the lobby owner we have to switch to the host scene.
	if Steam.getLobbyOwner(lobby_id) == Steam.getSteamID():
		InstanceManager.game_root.change_3d_scene("uid://dlngvgv71ebh4", true, false, SceneTransition.FADE_TO_BLACK)
	else:
		InstanceManager.game_root.change_3d_scene("uid://dgtgevitn80jx", true, false, SceneTransition.FADE_TO_BLACK)
